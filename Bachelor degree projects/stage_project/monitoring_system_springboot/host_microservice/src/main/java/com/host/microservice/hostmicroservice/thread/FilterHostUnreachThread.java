...
public class FilterHostUnreachThread extends Thread{
    ...
    public FilterHostUnreachThread(String name, CountDownLatch latch, String[] contactgroups, ArrayList<HostDto> host,
    ...
    public void run() {
        ...
        try {
            ...
            if(isSuperAdmin){
                query ="{" + lqlQueryCreator.filterHostByStatus2()+"}";
            }

            else {
                query = "{" + lqlQueryCreator.filterHostByContactgroupAndState2Query(contactgroups) + "}";

            }


            System.out.println("unreach query"+ query);
