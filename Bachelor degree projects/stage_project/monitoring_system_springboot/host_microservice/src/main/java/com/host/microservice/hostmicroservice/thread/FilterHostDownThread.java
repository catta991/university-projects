...
public class FilterHostDownThread extends Thread{
   ...
   private RestTemplate restTemplate = new RestTemplate();
    ...
    public FilterHostDownThread(String name, CountDownLatch latch, String[] contactgroups, ArrayList<HostDto> host,
    ...
    public void run() {
        ...
        try {
            ...
            if(isSuperAdmin){
                query ="{" + lqlQueryCreator.filterHostByStatus1()+"}";
            }

            else {
                 query = "{" + lqlQueryCreator.filterHostByContactgroupAndState1Query(contactgroups) + "}";

            }

            System.out.println("down query"+ query);
            System.out.println(url);

            ResponseEntity<HostsArray> filterHostDownResp = restTemplate.exchange(
                    url,
                    HttpMethod.GET, body, HostsArray.class, query);

            List<HostDto> dtos = filterHostDownResp.getBody().getValue()
                    .stream()
            ...
            latch.countDown();
            System.out.println("down hosts thread " +host);
